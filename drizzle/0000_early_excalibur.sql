...
CREATE TABLE "jvcritique_post" (
	"id" integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (sequence name "jvcritique_post_id_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START WITH 1 CACHE 1),
	"name" varchar(256),
...
ALTER TABLE "jvcritique_post" ADD CONSTRAINT "jvcritique_post_createdById_jvcritique_user_id_fk" FOREIGN KEY ("createdById") REFERENCES "public"."jvcritique_user"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "created_by_idx" ON "jvcritique_post" USING btree ("createdById");--> statement-breakpoint
CREATE INDEX "name_idx" ON "jvcritique_post" USING btree ("name");--> statement-breakpoint
